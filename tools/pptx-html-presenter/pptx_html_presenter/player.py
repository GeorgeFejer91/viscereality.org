...
(() => {
  ...
  function lerpState(from, to, progress, transition) {
    if (!from) {
      const fadeProgress = unmatchedFadeProgress(progress, transition, "enter", to?.trackId || null);
      const state = structuredClone(to);
      state.opacity = (to.opacity ?? 1) * fadeProgress;
      return state;
    }
    if (!to) {
      const fadeProgress = unmatchedFadeProgress(progress, transition, "exit", from?.trackId || null);
  ...
  function unmatchedFadeProgress(progress, transition, direction, trackId = null) {
    const runtime = scene.runtime || {};
    if (runtime.fadeUnmatched === false) {
      return progress >= 1 ? 1 : 0;
    }
    const fade = transition?.unmatchedFade || {};
    const trackFade = trackId && fade.tracks ? (fade.tracks[trackId] || {}) : {};
    const prefix = direction === "exit" ? "exit" : "enter";
    const rawStart = Number(trackFade[`${prefix}Start`] ?? fade[`${prefix}Start`] ?? runtime.unmatchedFadeStart ?? 0);
    const rawEnd = Number(trackFade[`${prefix}End`] ?? fade[`${prefix}End`] ?? runtime.unmatchedFadeEnd ?? 1);
  ...
  function runTransition(fromIndex, toIndex, fixedProgress, captureOptions = null) {
    ...
    let transition = transitionFor(fromIndex, toIndex) || { durationSec: 0 };
    transition = transitionWithCaptureOverrides(transition, captureOptions);
  ...
  function transitionWithCaptureOverrides(transition, captureOptions) {
    const override = captureOptions?.unmatchedFadeOverride || null;
    if (!override) return transition;
    return {
      ...transition,
      unmatchedFade: {
        ...(transition?.unmatchedFade || {}),
        ...override,
        tracks: {
          ...((transition?.unmatchedFade || {}).tracks || {}),
          ...(override.tracks || {}),
        },
      },
    };
